...
test("authority-client rejects mixed configuration and resumes one acknowledged ticket without local capability", async () => {
  ...
  const lease = { leaseId: "55555555-5555-4555-8555-555555555555", generation: 1, renewSequence: 0, renewByEpochMs: Date.now() + 60_000 };
  const ticketFor = (id, ticketRequestId, state, revision, ticketLease = lease) => ({ schemaVersion: 1, ticketId: id, requestId: ticketRequestId, provider: "anthropic-a", state, revision, lease: ticketLease });
  const healthBody = { schemaVersion: 1, protocolVersion: 2, authorityId: config.expectedAuthorityId, instanceId: "33333333-3333-4333-8333-333333333333", provider: "anthropic-a", port: 8791, stateSchemaVersion: 2, status: "ready" };
