...
test("failed acquisition remains pending and later recovery returns a permit", async () => {
  let attempts = 0; let ensureCalls = 0; let warnings = 0; let transportCalls = 0; const never = new Promise(() => {});
  const pending = (async () => { await acquirePermitResponse(19000, { session: "test", provider: "anthropic" }, "/unused", { request: async () => { attempts++; return {}; }, ensure: async () => { ensureCalls++; throw new Error("port occupied"); }, wait: async () => attempts >= 3 ? never : undefined, warningAfterAttempts: 3, retryMs: 1, onUnavailable: (message) => { warnings++; assert.match(message, /provider request remains blocked/i); } }); transportCalls++; })();
  await new Promise((resolve) => setImmediate(resolve));
  assert.equal(attempts, 3); assert.equal(ensureCalls, 3); assert.equal(warnings, 1); assert.equal(transportCalls, 0);
  void pending;
