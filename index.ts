...
// Turning bypass on must reach requests that are already waiting on the authority. Each Pi session
// rereads the bypass file before a new request, but a request already queued or retrying against an
// unreachable host would otherwise keep waiting for the host to return. The watcher polls the same
// file while a shared acquisition or completion is in flight and aborts it with a distinct reason,
// so the caller can move the request to local gating instead of treating it as a cancelled turn.
const BYPASS_POLL_MS = 1_000;
class BypassDetachError extends Error { constructor() { super("bypass detached this machine from the authority"); this.name = "BypassDetach"; } }
function watchBypass(detached: (() => boolean) | undefined, signal?: AbortSignal) {
  ...
  const timer = detached ? setInterval(() => { if (!controller.signal.aborted && detached()) controller.abort(new BypassDetachError()); }, BYPASS_POLL_MS) : undefined; timer?.unref?.();
  return { signal: controller.signal, detachedByBypass: () => controller.signal.reason instanceof BypassDetachError, stop: () => { if (timer) clearInterval(timer); signal?.removeEventListener("abort", forward); } };
}
...
type TicketOperation = { action: "claim" | "cancel" | "renew" | "complete"; operationId: string; additions: Record<string, unknown> };
type UnresolvedTicket = { provider: string; requestId: string; sessionId: string; createdAtEpochMs: number; ticket?: Ticket; operation?: TicketOperation; completed?: boolean };
...
type LedgerMutation = (mutation: (ledger: AuthorityLedger) => void | Promise<void>) => Promise<void>;
type AuthorityClientOptions = { request?: AuthorityRequest; token?: (reference: KeychainReference) => Promise<string>; readLedger?: () => Promise<AuthorityLedger>; writeLedger?: (ledger: AuthorityLedger) => Promise<void>; mutateLedger?: LedgerMutation; sessionId?: string; wait?: (ms: number, signal?: AbortSignal) => Promise<void> };
...
function authorityConfigFromFile(file: string): Omit<AuthorityClientConfig, "statePath"> {
  ...
  // `proberInstallationId` is the allowance prober's own per-machine publishing identity. The gate
  // never publishes under it and so never reads it, but an unrecognised key fails the whole
  // configuration, which takes every Pi session and every monitor lane down with it.
  if (!isRecord(parsed) || !hasOnlyKeys(parsed, ["schemaVersion", "mode", "origin", "expectedAuthorityId", "installationId", "proberInstallationId", "keychain", "monitorSource", "publisherEnabled", "lanes"])) configError("configuration fields are invalid");
  if (parsed.schemaVersion !== 1 || parsed.mode !== "authority-client" || !validOrigin(parsed.origin) || !isUuid(parsed.expectedAuthorityId) || !isUuid(parsed.installationId) || (parsed.proberInstallationId !== undefined && !isUuid(parsed.proberInstallationId)) || !isRecord(parsed.keychain) || !hasOnlyKeys(parsed.keychain, ["permitMutate", "snapshotRead", "allowancePublish"]) || !validReference(parsed.keychain.permitMutate) || !validReference(parsed.keychain.snapshotRead) || !validReference(parsed.keychain.allowancePublish) || !["local", "authority"].includes(String(parsed.monitorSource)) || typeof parsed.publisherEnabled !== "boolean" || !isRecord(parsed.lanes) || !hasOnlyKeys(parsed.lanes, AUTHORITY_PROVIDERS)) configError("configuration fields are invalid");
...
export function defaultLedgerWriter(file: string): (ledger: AuthorityLedger) => Promise<void> { return async (ledger) => { const directory = path.dirname(file); const temporary = `${file}.${process.pid}.${crypto.randomUUID()}.tmp`; const payload = JSON.stringify(ledger); let handle: fs.promises.FileHandle | undefined; try { await fsp.mkdir(directory, { recursive: true, mode: 0o700 }); handle = await fsp.open(temporary, "wx", 0o600); await handle.writeFile(payload, "utf8"); await handle.sync(); await handle.close(); handle = undefined; await fsp.rename(temporary, file); const directoryHandle = await fsp.open(directory, "r"); try { await directoryHandle.sync(); } finally { await directoryHandle.close(); } } catch (error) { await handle?.close().catch(() => {}); await fsp.unlink(temporary).catch(() => {}); throw error; } }; }
// A session killed mid-write leaves its lock directory behind, and nothing else ever removes it, so
// every later acquire on that machine fails with "authority retry state is locked" until an operator
// deletes it by hand. No holder can outlive this window by legitimate means: the lock is held only
// across one read, mutate, and write of a small local file. A lock older than the window is
// therefore an orphan and is broken so the process can proceed.
const LEDGER_LOCK_STALE_MS = 30_000;
function defaultLedgerMutator(file: string): LedgerMutation { const read = defaultLedgerReader(file); const write = defaultLedgerWriter(file); const lock = `${file}.lock`; return async (mutation) => { await fsp.mkdir(path.dirname(file), { recursive: true, mode: 0o700 }); for (let attempt = 0; attempt < 500; attempt++) { try { await fsp.mkdir(lock, { mode: 0o700 }); break; } catch (error: any) { if (error?.code !== "EEXIST") throw error; const orphaned = await fsp.stat(lock).then((stat) => Date.now() - stat.mtimeMs > LEDGER_LOCK_STALE_MS).catch(() => false); if (orphaned) { await fsp.rmdir(lock).catch(() => {}); continue; } if (attempt === 499) throw new Error("authority retry state is locked"); await sleep(10); } } try { const ledger = await read(); await mutation(ledger); await write(ledger); } finally { await fsp.rmdir(lock).catch(() => {}); } }; }
...
const TICKET_KEYS = ["schemaVersion", "ticketId", "requestId", "provider", "state", "revision", "createdAtEpochMs", "enqueuedAtEpochMs", "offeredAtEpochMs", "offerExpiresAtEpochMs", "terminalAtEpochMs", "terminalReason", "queueAhead", "lease"];
const LEASE_KEYS = ["leaseId", "generation", "claimedAtEpochMs", "renewSequence", "renewByEpochMs", "serverDeadlineEpochMs"];
function safeInteger(value: unknown, minimum = 0) { return Number.isSafeInteger(value) && (value as number) >= minimum; }
function epochMs(value: unknown) { return safeInteger(value); }
function validLease(value: unknown): boolean { return isRecord(value) && hasOnlyKeys(value, LEASE_KEYS) && isUuid(value.leaseId) && safeInteger(value.generation, 1) && epochMs(value.claimedAtEpochMs) && safeInteger(value.renewSequence) && epochMs(value.renewByEpochMs) && epochMs(value.serverDeadlineEpochMs) && value.renewByEpochMs >= value.claimedAtEpochMs && value.serverDeadlineEpochMs >= value.renewByEpochMs; }
// createdAtEpochMs comes from the client clock (echoed back) and enqueuedAtEpochMs from the
// authority clock, so their ordering is not a client-side invariant: an authority host running
// slightly behind the client legitimately returns enqueuedAtEpochMs < createdAtEpochMs within the
// authority's accepted skew window. Requiring the ordering rejected every create on a skewed host
// forever, since it is not stale state the discard-and-retry recovery can clear.
function validTicket(value: unknown, record: UnresolvedTicket): value is Ticket { if (!isRecord(value) || !hasOnlyKeys(value, TICKET_KEYS) || value.schemaVersion !== 1 || !isUuid(value.ticketId) || value.requestId !== record.requestId || value.provider !== record.provider || !TICKET_STATES.has(value.state) || !safeInteger(value.revision, 1) || !epochMs(value.createdAtEpochMs) || !epochMs(value.enqueuedAtEpochMs) || !safeInteger(value.queueAhead) || value.queueAhead > 4096 || !Object.hasOwn(value, "lease")) return false; const offered = value.offeredAtEpochMs; const expiry = value.offerExpiresAtEpochMs; const terminal = value.terminalAtEpochMs; if ((offered !== null && !epochMs(offered)) || (expiry !== null && !epochMs(expiry)) || (terminal !== null && !epochMs(terminal)) || (offered === null) !== (expiry === null) || (offered !== null && expiry < offered)) return false; const previous = record.ticket; if (previous && (value.ticketId !== previous.ticketId || value.createdAtEpochMs !== previous.createdAtEpochMs || value.enqueuedAtEpochMs !== previous.enqueuedAtEpochMs || value.revision < previous.revision)) return false; if (value.state === "queued") return offered === null && expiry === null && terminal === null && value.terminalReason === null && value.lease === null; if (value.state === "offered") return offered !== null && expiry !== null && terminal === null && value.terminalReason === null && value.lease === null; if (value.state === "active" || value.state === "uncertain") return offered !== null && expiry !== null && terminal === null && value.terminalReason === null && validLease(value.lease); const reasons = value.state === "cancelled" ? ["client_cancelled", "authority_draining"] : value.state === "released" ? ["released", "operator_reconciled"] : value.state === "throttled" ? ["assistant_rate_limit", "assistant_overloaded"] : ["offer_expired"]; return terminal !== null && value.lease === null && reasons.includes(value.terminalReason); }
...
export function createAuthorityClient(config: AuthorityClientConfig, options: AuthorityClientOptions = {}) {
  const request = options.request ?? defaultAuthorityRequest(config.origin); const token = options.token ?? keychainToken; const readLedger = options.readLedger ?? defaultLedgerReader(config.statePath); const writeLedger = options.writeLedger ?? defaultLedgerWriter(config.statePath); const mutateLedger = options.mutateLedger ?? (options.readLedger || options.writeLedger ? async (mutation: (ledger: AuthorityLedger) => void | Promise<void>) => { const ledger = await readLedger(); await mutation(ledger); await writeLedger(ledger); } : defaultLedgerMutator(config.statePath)); const wait = options.wait ?? waitForRetry; const acquisitions = new Map<string, Promise<UnresolvedTicket>>(); const completions = new Map<string, Promise<void>>(); const unboundSessionId = crypto.randomUUID();
  const recordKey = (record: UnresolvedTicket) => `${record.sessionId}:${record.requestId}`;
  const save = async (record: UnresolvedTicket) => { await mutateLedger((ledger) => { for (const [key, candidate] of Object.entries(ledger.tickets)) if (candidate.sessionId === record.sessionId && candidate.requestId === record.requestId) delete ledger.tickets[key]; ledger.tickets[recordKey(record)] = record; }); };
  const remove = async (record: UnresolvedTicket) => { await mutateLedger((ledger) => { for (const [key, candidate] of Object.entries(ledger.tickets)) if (candidate.sessionId === record.sessionId && candidate.requestId === record.requestId) delete ledger.tickets[key]; }); };
  // Whether the record came from disk decides if it can be discarded on failure: a freshly minted
  // record cannot be the cause, so only a persisted one is ever thrown away.
  const loadOrCreate = async (provider: string): Promise<{ record: UnresolvedTicket; persisted: boolean }> => { const ownerSessionId = options.sessionId ?? (isUuid(sessionId) ? sessionId : unboundSessionId); let record!: UnresolvedTicket; let persisted = false; await mutateLedger((ledger) => { const existing = Object.values(ledger.tickets).find((candidate) => candidate.provider === provider && candidate.sessionId === ownerSessionId); persisted = !!existing; record = existing ?? { provider, requestId: crypto.randomUUID(), sessionId: ownerSessionId, createdAtEpochMs: Date.now() }; ledger.tickets[recordKey(record)] = record; }); return { record, persisted }; };
  const send = async (method: string, provider: string, pathname: string, body?: any, signal?: AbortSignal): Promise<any> => { const authorization = `Bearer ${await token(config.keychain.permitMutate)}`; const response = await request({ method, provider, port: config.lanes[provider as keyof AuthorityClientConfig["lanes"]].port, pathname, body, authorization, signal }); if (response.status >= 200 && response.status < 300) return response.body; throw new AuthorityResponseError(response); };
  ...
  const sendOperation = async (provider: string, pathname: string, body: any, signal?: AbortSignal): Promise<any> => { for (let attempt = 1; ; attempt++) { if (isAborted(signal)) throw abortError(); try { return await send("POST", provider, pathname, body, signal); } catch (error) { if (isAborted(signal) || (error as Error).name === "AbortError") throw abortError(); const failure = error instanceof AuthorityResponseError ? error : new AuthorityResponseError(); if (!failure.response || !failure.retryable) throw failure; await wait(retryDelay(failure, attempt), signal); } } };
  ...
  const operation = async (provider: string, record: UnresolvedTicket, action: TicketOperation["action"], additions: Record<string, unknown>, signal?: AbortSignal): Promise<Ticket> => { if (!record.ticket || !validTicket(record.ticket, record)) throw new Error("authority retry state is invalid"); const prior = record.operation?.action === action ? record.operation : undefined; const operationId = prior?.operationId ?? crypto.randomUUID(); const retryAdditions = prior?.additions ?? additions; record.operation = { action, operationId, additions: retryAdditions }; await save(record); const body = () => ({ schemaVersion: 1, operationId, expectedRevision: record.ticket!.revision, installationId: config.installationId, provider, accountBindingId: config.lanes[provider as keyof AuthorityClientConfig["lanes"]].accountBindingId, ...retryAdditions });
    try { const result = await sendOperation(provider, `/v1/tickets/${record.ticket.ticketId}/${action}`, body(), signal); if (!validTicket(result, record)) throw new Error("authority ticket response is invalid"); record.ticket = result; record.operation = undefined; await save(record); return result; } catch (error) { if (isAborted(signal) || (error as Error).name === "AbortError") throw abortError(); if (error instanceof AuthorityResponseError && error.response?.status === 409 && error.response.body?.error?.code === "stale_revision") { const current = await get(provider, record, signal); const completed = action === "complete" && ["released", "throttled"].includes(current.state); const claimed = action === "claim" && ["active", "uncertain"].includes(current.state); const cancelled = action === "cancel" && current.state === "cancelled"; const renewed = action === "renew" && current.lease?.renewSequence === retryAdditions.renewSequence; if (completed || claimed || cancelled || renewed) { record.operation = undefined; await save(record); return current; } record.operation = { action, operationId, additions: retryAdditions }; await save(record); return operation(provider, record, action, retryAdditions, signal); }
      ...
      const current = await get(provider, record, signal).catch(() => undefined); const completed = action === "complete" && ["released", "throttled"].includes(current?.state); const claimed = action === "claim" && ["active", "uncertain"].includes(current?.state); const cancelled = action === "cancel" && current?.state === "cancelled"; const renewed = action === "renew" && current?.lease?.renewSequence === retryAdditions.renewSequence; if (completed || claimed || cancelled || renewed) { record.operation = undefined; await save(record); return current!; } return operation(provider, record, action, additions, signal); }
  };
  const operationAcknowledged = (action: TicketOperation["action"], ticket: Ticket, additions: Record<string, unknown>) => (action === "complete" && ["released", "throttled"].includes(ticket.state)) || (action === "claim" && ["active", "uncertain"].includes(ticket.state)) || (action === "cancel" && ticket.state === "cancelled") || (action === "renew" && ticket.lease?.renewSequence === additions.renewSequence);
  const reconcileOperation = async (provider: string, record: UnresolvedTicket, signal?: AbortSignal) => { const pending = record.operation; if (!pending) return; if (!isRecord(pending.additions)) throw new Error("authority retry state is invalid"); const current = await get(provider, record, signal); if (operationAcknowledged(pending.action, current, pending.additions)) { record.operation = undefined; await save(record); return; } await operation(provider, record, pending.action, pending.additions, signal); };
  const cleanupAbortedAcquire = async (provider: string, record: UnresolvedTicket | undefined) => { if (!record?.ticket) return; const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 5_000); timeout.unref?.(); try { if (["complete", "cancel"].includes(record.operation?.action ?? "")) await reconcileOperation(provider, record, controller.signal); const current = await get(provider, record, controller.signal); if (["queued", "offered"].includes(current.state)) { const cancelled = await operation(provider, record, "cancel", {}, controller.signal); if (cancelled.state === "cancelled") await remove(record); } else if (["active", "uncertain"].includes(current.state)) { const lease = current.lease; if (!validLease(lease)) throw new Error("authority lease is invalid"); const completed = await operation(provider, record, "complete", { leaseId: lease.leaseId, generation: lease.generation, outcome: "released", reason: null }, controller.signal); if (["released", "throttled"].includes(completed.state)) await remove(record); } } catch {} finally { clearTimeout(timeout); } };
  // Persisted retry state can outlive the ticket it names: an authority reinstall bumps the lane
  // term and a reclaimed ticket eventually compacts away. The record is then unusable, and every
  // later acquire rebuilds from it and fails the same way, so a session waits for a permit forever.
  // Discarding the stale record and retrying once from a clean one recovers without operator help.
  // Only one retry is allowed, so a genuine authority fault still surfaces instead of looping.
  const acquireOne = async (provider: string, signal?: AbortSignal, staleRecordDiscarded = false): Promise<UnresolvedTicket> => { let record: UnresolvedTicket | undefined; let recordWasPersisted = false; try { await health(provider, signal); const loaded = await loadOrCreate(provider); record = loaded.record; recordWasPersisted = loaded.persisted; const persistedTicket = record.ticket;
      await reconcileOperation(provider, record, signal);
      if (persistedTicket && !record.operation && ["active", "uncertain"].includes(record.ticket?.state)) await get(provider, record, signal);
      ...
      for (;;) { if (isAborted(signal)) throw abortError(); const ticket = record.ticket!; if (ticket.state === "active" || ticket.state === "uncertain") return record; if (ticket.state === "offered") { const claimed = await operation(provider, record, "claim", {}, signal); if (isAborted(signal)) throw abortError(); if (claimed.state === "active" || claimed.state === "uncertain") return record; } else if (["cancelled", "released", "throttled", "offerExpired"].includes(ticket.state)) { await remove(record); return acquireOne(provider, signal); } else { await wait(RETRY_MS, signal); await get(provider, record, signal); } }
    } catch (error) { if (isAborted(signal) || (error as Error).name === "AbortError") { if (signal?.reason instanceof BypassDetachError) void cleanupAbortedAcquire(provider, record); else await cleanupAbortedAcquire(provider, record); throw abortError(); }
      // A persisted record can be unusable in ways that only surface on the create path, where it
      // holds no ticket and no pending operation: a reused request id whose original ticket the
      // authority has since compacted still fails validation against the response it returns.
      const unusableRecord = !staleRecordDiscarded && !!record && recordWasPersisted && (error instanceof AuthorityResponseError ? !error.retryable : error instanceof Error && ["authority create response is invalid", "authority ticket identity is invalid", "authority retry state is invalid"].includes(error.message));
      if (unusableRecord) { await remove(record!); return acquireOne(provider, signal, true); }
      throw error; } };
  const acquire = async (provider: string, signal?: AbortSignal): Promise<UnresolvedTicket> => { if (!(AUTHORITY_PROVIDERS as readonly string[]).includes(provider)) throw new Error("provider is not configured for authority mode"); const completion = completions.get(provider); if (completion) { await completion; if (completions.get(provider) === completion) completions.delete(provider); return acquire(provider, signal); } const existing = acquisitions.get(provider); if (existing) return existing; const pending = acquireOne(provider, signal); acquisitions.set(provider, pending); void pending.catch(() => { if (acquisitions.get(provider) === pending) acquisitions.delete(provider); }); return pending; };
  ...
  // A lease can be retired by the authority while a slow request is still finishing: the client
  // stops renewing, the lease is reclaimed, and the slot returns to the queue. When that request
...
let authorityCompletion: Promise<void> | undefined;
function beginAuthorityLifecycle() { let settle!: () => void; authorityLifecycle = new Promise<void>((resolve) => { settle = resolve; }); settleAuthorityLifecycle = () => { const finish = settle; authorityLifecycle = undefined; settleAuthorityLifecycle = undefined; finish(); }; }
function endAuthorityLifecycle() { settleAuthorityLifecycle?.(); }
// Cancelling a turn is an ordinary outcome, not a gate fault. The abort rejection that ends an
// in-flight acquisition must stay inside the gate: surfacing it from the provider hook printed an
// extension error with a stack, and left the "waiting for shared permit" status on screen forever,
// so an aborted turn looked like a hung gate. The ticket is already cancelled or completed by the
// client's own abort cleanup, so the only work left here is to clear the waiting status.
// A request waiting on the authority moves to local gating as soon as bypass turns on. The ticket is
// cancelled in the background when the host is reachable, and otherwise expires on the authority.
export async function acquireAuthority(ctx: any, client: ReturnType<typeof createAuthorityClient>, provider: string, detached?: () => boolean): Promise<"acquired" | "aborted" | "bypassed"> { if (isAborted(ctx.signal)) return "aborted"; while (authorityLifecycle) await authorityLifecycle; if (detached?.()) return "bypassed"; beginAuthorityLifecycle(); ctx.ui?.setStatus?.("claude-permit-gate", "Claude: waiting for shared permit..."); const watch = watchBypass(detached, ctx.signal); try { const record = await client.acquire(provider, watch.signal); if (isAborted(ctx.signal)) { await client.cancel(record).catch(() => {}); endAuthorityLifecycle(); ctx.ui?.setStatus?.("claude-permit-gate", "Claude gate: ready"); return "aborted"; } const permit: AuthorityPermit = { kind: "authority", client, record }; activePermit = permit; const interval = Math.max(5_000, Math.min(60_000, Math.floor(Number(record.ticket?.lease?.renewByEpochMs ?? Date.now() + 15_000) - Date.now()) / 2)); permit.renewTimer = setInterval(() => { if (activePermit === permit) void client.renew(record).catch(() => {}); }, interval); permit.renewTimer.unref?.(); ctx.ui?.setStatus?.("claude-permit-gate", "Claude: shared permit active"); return "acquired"; } catch (error) { endAuthorityLifecycle(); if (watch.detachedByBypass()) { ctx.ui?.setStatus?.("claude-permit-gate", "Claude gate: ready (bypass)"); return "bypassed"; } if (isAborted(ctx.signal) || (error as Error)?.name === "AbortError") { ctx.ui?.setStatus?.("claude-permit-gate", "Claude gate: ready"); return "aborted"; } throw error; } finally { watch.stop(); } }
...
// Completing a shared lease retries until the authority answers, so a host that went down would
// hold this session's lane forever. Once bypass is on, the completion gives way and the lease is
// dropped locally for the authority to reclaim on its renewal deadline.
async function release(throttle: boolean, reason: string, cooldownMs?: number, detached?: () => boolean) { const permit = activePermit; if (!permit) return; if (permit.kind === "authority") { if (authorityCompletion) return authorityCompletion; const failure = throttle ? (reason.includes("overloaded") ? "overloaded" : "rate-limit") : undefined; authorityCompletion = (async () => { const watch = watchBypass(detached); try { await permit.client.complete(permit.record, failure, watch.signal); } catch (error) { if (!watch.detachedByBypass()) throw error; await permit.client.abandon(permit.record); } finally { watch.stop(); } if (permit.renewTimer) clearInterval(permit.renewTimer); activePermit = undefined; endAuthorityLifecycle(); })(); try { await authorityCompletion; } finally { authorityCompletion = undefined; } return; } activePermit = undefined; if (permit.renewTimer) clearInterval(permit.renewTimer); try { await postJson(permit.port, throttle ? "/throttle" : "/release", { permitId: permit.permitId, reason, cooldownMs }, 5000); } catch {} }
...
export default function (pi: ExtensionAPI) {
  ...
  const detached = bypassFile ? () => bypassState().enabled : undefined;
  ...
  pi.on("agent_end", async () => { await release(false, "agent-end", undefined, detached); }); pi.on("session_shutdown", async () => { await release(false, "session-shutdown", undefined, detached); });
