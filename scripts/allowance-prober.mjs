...
// exactly when its numbers matter most. The prober uses Pi's own OAuth resolver for every lane,
// including its locked refresh transaction, before polling usage.
//
...
import { createAllowanceAuth } from "./allowance-prober-auth.mjs";
...
const resolveToken = await createAllowanceAuth(AUTH_FILE).catch(() => undefined);
...
for (const provider of PROVIDERS) {
  ...
  let token;
  try { token = await resolveToken(provider); }
  catch {
    recordCredentialFailure(provider, "OAuth credential resolution failed");
    results.push(`${provider}: OAuth credential resolution failed`);
    continue;
  }
  if (!token) {
    recordCredentialFailure(provider, "OAuth credential unavailable");
    results.push(`${provider}: OAuth credential unavailable`);
    continue;
  }
  ...
  try {
    ...
    results.push(`${provider}: usage or publication request failed`);
  }
}
