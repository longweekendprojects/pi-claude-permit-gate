...
- `clientMode=authority-client` requires complete authority configuration before any Pi hook registers. While it is serving shared requests it has no loopback probe, daemon spawn, or local fallback capability: authority loss blocks the request rather than degrading to local truth.
- Operator bypass is the only exception, and it is a mode change rather than a fallback. An explicit, durable, owner-only operator toggle on one client detaches that machine from the authority and returns it to `clientMode=local` behavior on the same lane ports. No authority response, timeout, transport error, or health state may set, clear, or infer it. A detached client performs no authority request, holds no lease, and is not counted in shared capacity; a lease it held when the operator detached is reclaimed by the authority's ordinary renewal deadline, as `uncertain` when the provider request cannot be fenced. Invalid, unreadable, or expired toggle state resolves to attached.
